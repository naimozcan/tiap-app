...
function ExceptionListRow({ exception }) {
  return (
    <div className="w-full min-w-[1400px] grid grid-cols-12 gap-0 p-2">
      ...
      <div className="h-[40px]col-span-1 flex items-center justify-start text-[11px] underline"><Link to={`/logs/${exception._id}`}>{exception.no.slice(0, 10)}...</Link></div>
      ...
      <div className="h-[40px]col-span-1 flex items-center justify-start text-[11px]">{exception.order.no.slice(0, 10)}...</div>
      ...
      <div className="h-[40px]col-span-1 flex items-center justify-start text-[11px] underline"><Link to={`/root-causes/${exception.rootcause._id}`}>{exception.rootcause.title.length < 12 ? exception.rootcause.title : `${exception.rootcause.title.slice(0, 12)}...`}</Link></div>
      ...
      <div className="h-[40px]col-span-1 flex items-center justify-start text-[11px]">{exception.errorBy === null ? "-" : exception.errorBy.split("@")[0]}</div>
